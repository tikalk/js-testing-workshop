...
TodoApp.ListView.prototype = {
	addActionsBar: function(){
		...
		this.$el.find('#add-new').on('click', this.addNew.bind(this));
		this.$el.find('#remove-todos').on('click', this.removeTodos.bind(this));
		this.$newTodo.on('keypress', this.handleKeyPress.bind(this));
	},

	handleKeyPress: function(ev){
		var ENTER = 13;
		if (ev.charCode === ENTER) {
			this.addNew();
		}	
	},
	...
	renderItem: function (itemModel) {
		...
		this.$list.append(this.views[itemModel.id].$el);
	},
